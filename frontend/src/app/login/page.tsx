...
export default function LoginPage() {
      ...
      if (response.data) {
        ...
        addToast({
          ...
          description: `Selamat datang, ${
            response.data.user.profile?.firstName || response.data.user.email
          }!`,
          variant: "default",
        });
  ...
  const retryConnection = async () => {
    ...
    try {
      const healthCheck = await apiClient.healthCheck();
      setIsConnected(healthCheck.success || false);

      if (healthCheck.success) {
        setErrors((prev) => ({ ...prev, general: "" }));
        addToast({
          title: "Koneksi Restored",
          description: "Backend server terhubung kembali",
          variant: "default",
        });
      } else {
        setConnectionError("Still cannot connect to backend server");
      }
    } catch (error) {
      setIsConnected(false);
