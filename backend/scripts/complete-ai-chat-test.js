...
async function completeAIChatTest() {
  ...
  try {
    ...
    if (!sessionResponse.ok) {
      throw new Error(
        `Session creation failed: ${await sessionResponse.text()}`
      );
    }
    ...
    const testMessage =
      "Bagaimana cara mengatur budget bulanan untuk fresh graduate dengan gaji 5 juta?";
    ...
    console.log(
      `   ⏱️  Processing time: ${messageData.aiMessage.metadata.processingTime}ms`
    );
    console.log("");
    ...
    if (!historyResponse.ok) {
      throw new Error(
        `History retrieval failed: ${await historyResponse.text()}`
      );
    }
    ...
    historyData.forEach((msg, index) => {
      console.log(
        `      ${index + 1}. ${msg.role}: "${msg.content.substring(0, 50)}..." (${msg.id})`
      );
    });
    console.log("");
    ...
    if (!sessionsResponse.ok) {
      throw new Error(
        `Sessions retrieval failed: ${await sessionsResponse.text()}`
      );
    }
    ...
    const hasRequiredFields = requiredMessageFields.every(
      (field) => sampleMessage[field]
    );

    // Test session structure
    const requiredSessionFields = ["id", "title", "type", "createdAt"];
    const sessionCompatible = requiredSessionFields.every(
      (field) => sessionData[field]
    );

    console.log(
      `   📝 Message structure compatible: ${hasRequiredFields ? "✅" : "❌"}`
    );
    console.log(
      `   💬 Session structure compatible: ${sessionCompatible ? "✅" : "❌"}`
    );
    ...
    console.log(
      `   🤖 AI response structure compatible: ${aiCompatible ? "✅" : "❌"}`
    );
    console.log("");
    ...
    return {
      ...
      results: {
        ...
        frontendCompatibility:
          aiCompatible && hasRequiredFields && sessionCompatible
            ? "✅ Working"
            : "⚠️ Issues",
      },
      testData: {
