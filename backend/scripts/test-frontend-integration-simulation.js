...
async function testFrontendIntegration() {
  ...
  try {
    ...
    const messageResponse = await apiClient.post(`chat/sessions/${sessionId}/messages`, {
    ...
    console.log('🤖 AI response preview:', messageData.aiMessage?.content?.substring(0, 100) + '...');

    // Get chat history
    console.log('5. 📜 Getting chat history...');
    const historyResponse = await apiClient.get(`chat/sessions/${sessionId}/messages`);
