...
async function validateFrontendIntegration() {
  ...
  try {
    ...
    const messageResponse = await authAxios.post(`chat/sessions/${sessionId}/messages`, {
    ...
    console.log('   AI response length:', messageData.aiMessage?.content?.length || 0, 'characters');

    // Test getting chat history
    console.log('4. 📜 Testing chat history retrieval...');
    const historyResponse = await authAxios.get(`chat/sessions/${sessionId}/messages`);
    ...
    const hasAllFields = requiredFields.every(field => sampleMessage.hasOwnProperty(field));
